...
{ 
    if($_SESSION["department"]!='Owner' &&$_SESSION["role"]!='manager' && $_SESSION["role"]!='Director')
        header("Location: ../");
...
<div id='batch_div' class="position-fixed d-none my-4 p-4 shadow list-group-item-light" style="top: 80%; left: 90%; z-index:1;">
    ...
    <?php
    $cond=($_SESSION['role']=='Owner'|| $_SESSION['role']=='Director');
    $query = "SELECT * FROM requests WHERE `next_step`='directors' AND $cond";
    ...
    if($result_agreement_fetch -> num_rows>0)
        ...
        {
            $query = "SELECT count(*) AS num FROM requests WHERE `next_step`='directors' AND $cond AND `request_type`='agreement'";
    ...
    <div class="row m-auto" id='reload'>
        <?php
        $sql = "SELECT * FROM requests WHERE `next_step`='directors' AND $cond group by request_type";
        $stmt_agreement_fetch_grouped = $conn->prepare($sql);
        $stmt_agreement_fetch_grouped -> execute();
        $result_agreement_fetch_grouped = $stmt_agreement_fetch_grouped -> get_result();
        $str="";
        $emp=true;
        if($result_agreement_fetch_grouped -> num_rows>0)
            while($r = $result_agreement_fetch_grouped -> fetch_assoc())
            {
                $sql = "SELECT * FROM requests WHERE `next_step`='directors' AND $cond AND 
                `request_type` = ? ORDER BY date_needed_by ASC";
                $stmt_agreement_fetch_filler = $conn->prepare($sql);
                $stmt_agreement_fetch_filler -> bind_param("s", $r['request_type']);
                $stmt_agreement_fetch_filler -> execute();
                $result_agreement_fetch_filler = $stmt_agreement_fetch_filler -> get_result();
                $str="";
                $emp=true;
                if($result_agreement_fetch_filler -> num_rows>0)
                    while($row = $result_agreement_fetch_filler -> fetch_assoc())
                    {
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="price_modalLabel">Set prcing</h5>
