...
{
    if(strpos($_SESSION["a_type"],"manager") === false && $_SESSION["role"] != "Director" && $_SESSION["role"] != "Disbursement") header("Location: ../");
    $string_inc = 'head.php';
    include $string_inc;
}
else
...
?>
<script>
    set_title("LPMS | Proccess Payment");
    sideactive("review");
...
<div id='batch_div' class="position-fixed d-none my-4 p-4 shadow bg-light" style="top: 80%; left: 90%; z-index:1;">
    <form method="GET" action="allphp.php">
        <div class=''>
            <button type='button' onclick = 'prompt_confirmation(this)' class='btn btn-xl btn-outline-primary shadow mt-3' name='batch_review' id='batch_review'>Reviewed</button>
        </div>
    </form>
            ...
            <div class='col-md-6 col-lg-4 col-xl-3 my-4 focus'>
                ...
                <form method='GET' action='allphp.php'>
                    <ul>
                        ...
                        <li class='mt-3'>
                        <button type='button' onclick = 'prompt_confirmation(this)' class='btn btn-outline-success' value = '$r_clus[id]' name='process_disbursement'>Reviewed</button>
                        </li>
                    </ul>
                </form>
                </div>
