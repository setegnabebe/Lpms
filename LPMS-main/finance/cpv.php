...
<div class="row">
    ...
    <?php
        ...
        $sql_cpv_finance = "SELECT *,C.id As cid FROM `cluster` AS C inner Join cheque_info AS CPV ON C.id = CPV.cluster_id AND C.finance_company = ?";
        $stmt_cpv_finance = $conn->prepare($sql_cpv_finance);  
        $stmt_cpv_finance -> bind_param("s", $_SESSION['company']);
        $stmt_cpv_finance -> execute();
        $result_cpv_finance = $stmt_cpv_finance -> get_result();
        if($result_cpv_finance->num_rows>0)
        while($r_clus = $result_cpv_finance->fetch_assoc())
        {
            $printpage = "
                <form method='GET' action='../requests/print.php' class='float-end'>
                    <button type='submit' class='btn btn-outline-secondary border-0' name='print' value='".$r_clus['cid'].":|:all'>
            ...
            <div class='col-md-6 col-lg-4 col-xl-3 my-4 focus'>
                ...
                CPV Number - ".$r_clus['cpv_no']."$printpage
                </h3>
                <form method='GET' action='allphp.php'>
                    <ul>
                        <li class='text-start'><span class='fw-bold'>To Company : </span><span class='text-primary'>".$r_clus['providing_company']."</span></li>
                        <li class='text-start'><span class='fw-bold'>For Company : </span><span class='text-primary'>".$r_clus['company']."</span></li>
                        <li class='text-start'><span class='fw-bold'>By Company : </span><span class='text-primary'>".$r_clus['cheque_company']."</span></li>
                        ...
                        <li class='mt-3'>
                        <!-- ".$r_clus['cid']."::-::".$r_clus['purchase_order_ids']."-->
                            <button class='mx-auto btn btn-outline-success btn-sm mb-3' type='button' onclick = 'pay_req(this,\"view\")' value='CPV-".$r_clus['cpv_no']."' name='prepare' data-bs-toggle='modal' data-bs-target='#pay_requ'>
                                View CPV
                            </button>
                        </li>
                    </ul>
