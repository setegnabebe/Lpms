    ...
    echo "
        <div class='pricing'>
            <div class='section-title text-center py-2  alert-primary rounded'>
                <h6 class='text-white'>POs To Be Sent to Finance</h4> 
            </div>
            <div class='row'>
                $str
            </div>
...
?>
<script>
    set_title("LPMS | Send to Finance");
    sideactive("additional");
    function batch_select(e)
    {
        ...
        document.getElementById("batch_send").value = selections;
        if(indicator)
...
<div class="row">
    <div class="col-sm-6 col-md-6 col-lg-7 col-xl-7"> 
        ...
        <h2>Send Purchase Orders to Finance</h2>
        <ol class="breadcrumb my-4">
            <li class="breadcrumb-item"><a href='index.php' style="text-decoration: none;">Home</a></li>
            <li class="breadcrumb-item active">Send Purchase Orders to Finance</li>
        </ol>
    </div>
...
<div id='batch_div' class="position-fixed d-none my-4 p-4 shadow bg-light" style="top: 80%; left: 90%; z-index:1;">
    <form method="GET" action="allphp.php">
        <div class=''>
            <button type='button' onclick = 'prompt_confirmation(this)' class='btn btn-xl btn-outline-primary shadow mt-3' name='batch_send' id='batch_send'>Send To Finance</button>
        </div>
    </form>
    ...
    <?php
        $str="";
        $sql_clus = "SELECT * FROM `cluster` where `status`='Approved' AND `procurement_company` = ?";
        $stmt_for_finance = $conn->prepare($sql_clus);
        $stmt_for_finance -> bind_param("s", $_SESSION['company']);
        $stmt_for_finance -> execute();
        $result_for_finance = $stmt_for_finance -> get_result();
        if($result_for_finance -> num_rows>0)
        while($r_clus = $result_for_finance -> fetch_assoc())
        {
            ...
            <div class='col-md-6 col-lg-4 col-xl-3 my-4 focus'>
                ...
                <ul>
                    ...
                    <li class='mt-3'>
                    <button class='btn btn-outline-success' name='process_to_finance' value='".$r_clus['id']."'>Send to Finance</button>
